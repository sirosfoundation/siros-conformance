...
function brandReportHtml(htmlFile, backUrl, runId) {
  ...
  // Inject CSS before </head> and nav after <body>
  html = html.replace('</head>', `<style>${navCss}</style>\n</head>`);
  html = html.replace(/<body[^>]*>/, (match) => `${match}\n${navHtml}`);

  // Also override body padding-top to account for sticky nav
  html = html.replace('</head>', '<style>body { padding-top: 0 !important; }</style>\n</head>');
